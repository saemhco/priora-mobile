...
class TriageCubit extends Cubit<TriageState> {
  TriageCubit({required this._triageRepository}) : super(TriageState()) {
    _initChat();
  }
      ...
      if (isReady) {
        ...
        emit(
          state.copyWith(
            ...
            suggestedSpecialty: result['suggestedSpecialty'] as String?,
            suggestedSpecialties: specs,
            patientSafeMessage: result['patientSafeMessage'] as String?,
          ),
        );
      } else {
        // Stay on step 2 (chat) and append questions as options in the chat
        final missingQuestionsList =
            (result['missingInfoQuestions'] as List<dynamic>?) ?? <dynamic>[];
        final title =
            (result['patientSafeMessage'] as String?) ??
            'Por favor, selecciona o responde lo siguiente:';

        final questionTexts = <String>[];
        for (final q in missingQuestionsList) {
  ...
  Future<void> submitChatAnswers(String accessToken) async {
    ...
    final updatedMessages = List<TriageMessage>.from(
      state.chatMessages,
    );
    ...
    void checkCompletion() {
      ...
      if (!isTimerFinished || (result == null && error == null)) {
        debugPrint('[debug] checkCompletion: returning early');
        return;
      }

      if (error != null) {
        debugPrint(
          '[debug] checkCompletion: emitting error and going to step 2',
        );
        emit(
          state.copyWith(
            currentStep: 2,
      ...
      if (result != null) {
        final isReady = result['ready'] == true;
        debugPrint('[debug] checkCompletion: isReady=$isReady');
        if (isReady) {
          final rawSpecialties = result['suggestedSpecialties'];
          var specs = <String>[];
          if (rawSpecialties is List) {
    ...
    _startProgressTimer(0, () {
      debugPrint('[debug] timer finished');
    ...
    try {
      ...
      debugPrint('[debug] sending continueTriage: $body');

      result = await _triageRepository.continueTriage(
        accessToken: accessToken,
        data: body,
      );
      debugPrint('[debug] continueTriage response: $result');
    } catch (e, stack) {
      error = e;
      debugPrint('[debug] continueTriage error: $e\n$stack');
    } finally {
      debugPrint('[debug] finally block');
      checkCompletion();
    }
  }
  ...
  void _startProgressTimer(int startVal, void Function() onComplete) {
