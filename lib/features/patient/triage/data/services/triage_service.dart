...
import 'package:priora/features/patient/triage/data/models/triage_history_item_dto.dart';

/// HTTP client of the patient's triage. Only makes API calls and serializes
/// responses into DTOs; no business logic.
class TriageService {
  TriageService(this._dio);
  ...
  Future<List<TriageHistoryItemDto>> getTriageHistory({
    required String accessToken,
  }) async {
    try {
      final response = await _dio.get<dynamic>(
      ...
      if (response.statusCode == 200) {
        ...
        return list
            .map(
              (item) => TriageHistoryItemDto.fromJson(
